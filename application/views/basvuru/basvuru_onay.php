...
<div class="col-lg-12" >
    ...
    <div class="card">
		<div class="card-body">
		<a href="<?=base_url()?>Stajbasvuru/onaylanmis_basvurulari_getir"><input type='submit' class='btn btn-sm btn-success' value='Onaylanmış Başvurular'/></a>
		<a href="<?=base_url()?>Stajbasvuru/reddedilmis_basvurulari_getir"><input type='submit' class='btn btn-sm btn-danger' value='Reddedilmiş Başvurular'/></a>
								 ...
								 if($row->staj_durum==0){
									...
									if($row->durum == 1)
										echo "<td><div class='col-md-12'> 
												<input type='hidden' id='s_bilgi' value='".$row->sbilgi_id."'>
												<input type='hidden' id='idim' value='".$row->id."'>
												<input type='hidden' id='ogr_no' value='".$row->ogrenci_no."'>
													...
													<a class='col-md-3'><button id='kayit_red' class='btn btn-danger btn-sm' /><i class='fa fa-times' aria-hidden='true'></i></button></a>
											</div></td>";
									else 
...
<script>
$('#example').on('click', '#kayit_red', function(){
	$sbilgi = $(this).parents("td").find( "#s_bilgi" ).val();
	$idim = $(this).parents("td").find( "#idim" ).val();
	$ogrno = $(this).parents("td").find( "#ogr_no" ).val();
	console.log($sbilgi)
        ssi_modal.confirm({
                    content: 'Başvuruyu reddetmek istediğinize emin misiniz?',
                    okBtn: {
                    ...
                    if (result){
						window.location.href = '<?=base_url();?>Stajbasvuru/basvuruyu_reddet/'+$sbilgi+'/'+$idim+'/'+$ogrno;
					}
                }
        );
